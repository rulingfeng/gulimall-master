...
public class RegisterServiceImpl implements RegisterService {
    ...
    public void sendCode(String phone) {
        // 尝试从redis中获取用户的验证码
        String key = AuthServerConstant.RANDOM_CODE_REDIS_PREFIX + phone;
        ...
        redisTemplate.opsForValue().set(key, val, AuthServerConstant.RANDOM_CODE_VALID_TIME, TimeUnit.MINUTES);

        // 调用远程服务发送验证码
        SmsSendCodeTO smsSendCodeTO = new SmsSendCodeTO();
        smsSendCodeTO.setPhone(phone);
        smsSendCodeTO.setCode(code);
        smsSendCodeTO.setTimeout(AuthServerConstant.RANDOM_CODE_VALID_TIME);
    ...
    public boolean register(RegisterVO registerVO) {
        // 验证验证码合法性
        // 从redis中获取用户的验证码
        String key = AuthServerConstant.RANDOM_CODE_REDIS_PREFIX + registerVO.getPhone();
        String codeWithTime = redisTemplate.opsForValue().get(key);
        // 匹配用户传过来的验证码
        // 匹配失败
